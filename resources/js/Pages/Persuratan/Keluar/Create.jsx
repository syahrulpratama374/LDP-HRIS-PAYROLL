...
export default function Create({ auth, templates, karyawans }) {
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Buat Draft Surat Baru</h2>}
        >
                ...
                <div className="max-w-3xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        <form onSubmit={submit} className="space-y-6">
                            ...
                            <div>
                                <InputLabel htmlFor="template_id" value="Jenis Surat (Template)" />
                                <select
                                    id="template_id"
                                ...
                                >
                                    ...
                                    {templates.map((template) => (
                                        <option key={template.id} value={template.id}>
                                            {template.nama_template} ({template.kode_surat})
                                        </option>
                                    ))}
                                </select>
                            ...
                            <div>
                                <InputLabel htmlFor="karyawan_id" value="Karyawan Tujuan" />
                                <select
                                    id="karyawan_id"
                                    className="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
                            ...
                            <div className="flex items-center gap-4 pt-4 border-t">
                                ...
                                <Link href={route('keluar.index')} className="text-sm text-gray-600 hover:text-gray-900 hover:underline">
                                    Batal
                                </Link>
                            </div>
                        </form>
